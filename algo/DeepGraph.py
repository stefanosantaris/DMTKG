...
class DeepGraph(Model):
    def __init__(self, eigvecs):
        super(DeepGraph, self).__init__()

        self.eigvecs = eigvecs
        self.convolutional_layer_1 = tf.keras.layers.Conv1D(
                    filters=self.eigvecs, kernel_size=3, strides=(2, 2), activation='relu'),
        self.dense_layer =  tf.layers.dense({units: 10, activation: 'softmax'}),


    def call(self, adjs):
        (VPos, VColors, ITris) = load_off(adjs)
        neigvecs = min(VPos.shape[0], opt.neigvecs)
        hks = get_hks(VPos, ITris, neigvecs, np.array([opt.t]))
        x = self.convolutional_layer_1(hks)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--input", type=str, required=True,
                        help="Path to OFF file for triangle mesh on which to compute the HKS")
    parser.add_argument("--output", type=str, required=True,
                        help="Path to OFF file which holds a colored mesh showing the HKS")
